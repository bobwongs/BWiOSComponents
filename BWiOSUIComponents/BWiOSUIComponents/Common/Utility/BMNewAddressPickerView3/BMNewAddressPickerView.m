...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selected = (number && number.integerValue == row);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *array = _addressArrayM[tableViewIndex];
    [_selectedTitleMutableArray insertObject:array[indexPath.row] atIndex:_selectedTitleMutableArray.count - 1];  // 加在倒数第二个位置
...
- (void)addNextTableView {
    ...
    _bottomScrollView.contentSize = CGSizeMake(CGRectGetMaxX(tableView.frame), CGRectGetHeight(_bottomScrollView.frame));
}
